...
void GLTriangulationWidget::cleanup()
{
    ...
    delete m_program;
    m_program = 0;
    m_inverseVbo.destroy();
    delete m_inverseProgram;
    m_inverseProgram = 0;
    doneCurrent();
}
...
void GLTriangulationWidget::initializeGL()
{
    ...
    m_inverseProgram->addShaderFromSourceCode(QOpenGLShader::Fragment, m_core ? fragmentShaderSourceCore : fragmentShaderSourceInverse);

    m_inverseProgram->bindAttributeLocation("vertex", 0);
    m_inverseProgram->bindAttributeLocation("normal", 1);
    m_inverseProgram->link();

    m_inverseProgram->bind();
    m_projMatrixLoc = m_inverseProgram->uniformLocation("projMatrix");
    m_mvMatrixLoc = m_inverseProgram->uniformLocation("mvMatrix");
    m_normalMatrixLoc = m_inverseProgram->uniformLocation("normalMatrix");
    m_lightPosLoc = m_inverseProgram->uniformLocation("lightPos");
