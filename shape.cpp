    ...
    : m_count(0)
{
    ...
    m_inv_data.resize((*mesh).get_number_triangles() * 3 * 6);
...
void Shape::add(SimpleMesh::Vertex & vertex, float * normal)
{
    ...
    *p++ = normal[2];
    ...
    //Add inverse faces
    GLfloat *p2 = m_inv_data.data() + m_count;
    *p2++ = (vertex.x() / (*mesh).get_diagonal());
    *p2++ = (vertex.y() / (*mesh).get_diagonal());
    *p2++ = (vertex.z() / (*mesh).get_diagonal());
    *p2++ = -normal[0];
    *p2++ = -normal[1];
    *p2++ = -normal[2];
